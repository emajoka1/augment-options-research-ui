...
def core_ready() -> tuple[bool, str]:
    if not CORE_PATH.exists():
        return False, f"Bundled core directory missing: {CORE_PATH}"
    if not MC_SCRIPT.exists():
        return False, f"Bundled workflow script missing: {MC_SCRIPT}"
    return True, "OK"
...
def run_mc_command(
    ...
    if not MC_SCRIPT.exists():
        return None, f"Missing script: {MC_SCRIPT}", 1
...
st.caption("Self-contained Streamlit dashboard with the workflow engine bundled into this repo.")
...
with st.sidebar:
    ...
    st.success("Bundled engine detected")
    st.divider()
...
runs_df = summarize_runs(load_runs(limit=200))
if runs_df.empty:
